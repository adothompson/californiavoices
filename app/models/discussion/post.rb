class Post < ActiveRecord::Base
  ...
  def self.build(options)
    raise ArgumentError.new unless options[:user] && options[:story] && options[:body]
    story = options.delete(:story)
    story.discussion ||= Discussion.create!(:discussable => story)
    post = story.discussion.posts.build(options)
    return post
  end
